...
import src
from src.core.managers.update import update_mgr
...
class UpdateDialog(tk.Toplevel):
    ...
    def _on_check_complete(self, has_update: bool, latest_version: Optional[str], release_notes: Optional[str]):
        ...
        if has_update and latest_version:
            ...
            self.latest_label.config(text=f"Latest Version: {src.__version__}")
            self.notes_text.config(state="normal")
            self.notes_text.delete("1.0", "end")
    ...
    def _update_progress(self, percent: int, current: int, total: int):
        """Update progress bar."""
    ...
    def _on_download_complete(self, download_path: Path):
